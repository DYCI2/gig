...
class Parameter(Generic[T]):
    """ Class for setting parameters that should be accessible over OSC.
        ...
        Note that the `setter` can be used if updating the given parameter should change additional values elsewhere,
        for example:
        >>> class SomeClass:
        >>>     def __init__(self):
        >>>         self.a: float
        >>>         self.b: Parameter[int] = Parameter("b", 0, setter=self.calculate_a)
        >>>
    ...
    def __init__(self,
        ...
        self.setter: Optional[Callable[[T], None]] = setter
